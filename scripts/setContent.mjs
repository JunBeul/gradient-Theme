...
function updateClock() {
  ...
  let dateString = `${today.getFullYear()}/${(today.getMonth(2) + 1)
    .toString()
    .padStart(2, "0")}/${today.getDate().toString().padStart(2, "0")}`;
  let timeString = `${today.getHours().toString().padStart(2, "0")}:${today
    .getMinutes()
    .toString()
    .padStart(2, "0")}`;
  ...
  if (keyElement) {
    ...
    if (soundSelect) {
      const switchSound = new Audio(
        "https://github.com/JunBeul/gradient-Theme/raw/refs/heads/main/source/switchSound.mp3"
      );
      switchSound.play();
    }
...
export async function simulateTyping(text, interval) {
  ...
  typingTask = (async () => {
    ...
    for (const char of text) {
      ...
      if (isUpperCase || isShiftRequired) {
        const shiftKeyElement = document.querySelector(
    ...
    typingTextElement.textContent = ""; // 글자 초기화
    // 엔터 누른 후 기다림
